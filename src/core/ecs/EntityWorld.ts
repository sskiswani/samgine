...
interface IMapping { [$key: string]: number; }

// - - - - - - - - - - - - - - - - - - - - -

export class EntityWorld {
    private _entityMap: IMapping = {};
    private _systemMap: IMapping = {};
    ...
    private _notifySystems(entity: IEntity) {
        this._systems.forEach((system) => {
            if (system.entities.get(entity.id)) {
                if (!system.interested(entity)) { system.remove(entity); }
                return;
            } else {
                if (system.interested(entity)) { system.insert(entity); }
            }
        });
    }

    public registerSystem(system: IEntitySystem) {
    ...
    public detachSystem(system: IEntitySystem | string) {
    ...
    // - - - - - - - - - - - - - - - - - - - - -
    //~ Entity Management
    public insertEntity(entity: IEntity) {
    ...
    public getEntity(identifier: number | string): IEntity {
    ...
    public destroyEntity(entity: IEntity | number | string) {
        let e: IEntity;
